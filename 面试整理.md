...
### 小程序
* 小程序主要api: 登录 wx.getuserInfo() wx.login(res) res有code 发往后台 支付wx.requestPayMent(), 参数包括 时间戳 字符串noncestr,package,signType,paySign
* 小程序的组件间关系：relations,会有周期linked,unlink ,指定type为'parent'或'child'//父子组件消失
* 数据绑定 直接写 xx = "{}"，事件触发: bind:fn = "childFn"